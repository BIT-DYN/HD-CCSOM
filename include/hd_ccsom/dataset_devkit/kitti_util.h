...
class KITTIData {
  public:
    ...
    void process_depth_img(const int scan_id, const cv::Mat& depth_img,
                           pcl::PointCloud<pcl::PointXYZL>& cloud, hd_ccsom::point3f& origin, bool reproject) {
    ...
    void reproject_imgs(const int current_scan_id, hd_ccsom::SemanticBKIOctoMap& map) {
    ...
    void reproject_img(const int scan_id, const int reproj_id, hd_ccsom::SemanticBKIOctoMap& map) {
      Eigen::Matrix4f transform = get_current_pose(scan_id);
      for (int32_t i = 0; i < im_width_ * im_height_; ++i) {
        int ux = i % im_width_;
        int uy = i / im_width_;
        ...
        if (pix_depth > 0.1) {
          ...
          hd_ccsom::SemanticOcTreeNode node = map.search(pt.x, pt.y, pt.z);
