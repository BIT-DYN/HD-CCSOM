...
namespace hd_ccsom {
    ...
    void SemanticBKIOctoMap::insert_map_point_single(pcl::PointCloud<MapPoint>::Ptr transformed_cells)
    {
        ...
        std::ofstream OutFile("/home/dyn/catkin_ws/src/HD-CCSOM/multi_result/single.txt"); 
...
#pragma omp critical
            ...
            if (if_cout)
            {
                if ( std::count(new_block.begin(), new_block.end(), key) == 0 )
                {
                    ...
                    if (probs[semantic] <= (1.0/SemanticOcTreeNode::num_class))   // 理论上是，即不是空的就用来比较，但是因为机器人1看到的共同区域距离较远，不确定性更高，所以筛选严格一点，如果换了数据集不一定一样
                        if_free_node = 1;
                    else
                        if_free_node = 0;
...
#pragma omp critical
#endif
            {
                ...
                {
                    // 注意有可能是前面的节点出来的block，所以要先判断是不是新的，这样的common_block才是正确的，这一块是最耗时的
                    // if ( std::count(new_block.begin(), new_block.end(), key) == 0 )
                    // if ( std::count(new_block.begin(), new_block.end(), key) == 0  && std::count(common_block.begin(), common_block.end(), key) == 0)
                    if ( (std::find(new_block.begin(), new_block.end(), key) ==  new_block.end() ) && (std::find(common_block.begin(), common_block.end(), key) == common_block.end())  )
            ...
            // ******为了比较*********，如果这个block本来就有（不是新增的），也就是重叠区域，再去看看这个node是不是新增的
            if (if_cout)
            {
                if ( std::count(common_block.begin(), common_block.end(), key) != 0 ) 
                {
                    std::vector<float> probs(SemanticOcTreeNode::num_class);
                    node.get_probs(probs);
                    // 找到最大的语义标签
                    ...
                    if ( if_free_node == 0 ) // 这个节点也是非空的时候才比较
                    {
                        ...
                        old_prob.push_back(probs[semantic]);
                        // 淘汰1：我自己定义的
                        // old_ent.push_back(1-probs[semantic] * log(nums_sum) /log(max_nms) );
                        // 淘汰2：信息熵的定义
                        float inf_ent = 0.0;
                        for (int sem_no = 0; sem_no < SemanticOcTreeNode::num_class; sem_no ++ )
                        {
                            if (probs[sem_no] < 0.01 || probs[sem_no] > 0.99)
                                continue;
                            else
                                inf_ent = inf_ent - probs[sem_no] *  log(probs[sem_no]) / log(2);
                        }
                        // old_ent.push_back(inf_ent);
                        // 正确：方差
                        std::vector<float> vars(SemanticOcTreeNode::num_class);
                        node.get_vars(vars);
                        old_ent.push_back(vars[semantic]);

                        // 另一个机器人的局部语义
                        float nums_sum_2;
                        nums_sum_2 = it->prob1+it->prob2+it->prob3;
                        if (nums_sum_2 > max_nms)
                            nums_sum_2 = max_nms;
                        old_semantic_2.push_back(it->semantic_label1);
                        old_prob_2.push_back(it->true_prob);
                        float inf_ent_2 = 0.0;
                        float pro_123 =  it->true_prob;
                        if (pro_123 > 0.01 && pro_123 < 0.99)
                            inf_ent_2 = inf_ent_2 - pro_123 * log (pro_123) / log(2);
                        pro_123 = it->true_prob * it->prob2 /  it->prob1;
                        if (pro_123 > 0.01 && pro_123 < 0.99)
                            inf_ent_2 = inf_ent_2 - pro_123 * log (pro_123) / log(2);
                        pro_123 = it->true_prob * it->prob3 /  it->prob1;
                        if (pro_123 > 0.01 &&pro_123 < 0.99)
                            inf_ent_2 = inf_ent_2 - pro_123 * log (pro_123) / log(2);
                        //淘汰2：信息熵
                        // old_ent_2.push_back(inf_ent_2);
                        // 淘汰1：自己定义的
                        // old_ent_2.push_back(1- it->true_prob * log(nums_sum_2) /log(max_nms) );
                        // 正确：方差
                        old_ent_2.push_back(  it->true_var );

                        hd_ccsom::point3f node_center;
                        node_center.x() = it->x;
                        node_center.y() = it->y;
                        node_center.z() = it->z;
            ...
            // ******为了比较*********，新融合的结果也存储下来，但这个是单对单融合的，我们的方法包含了，不打算对比了
            if (if_cout)
            {
                if ( std::count(common_block.begin(), common_block.end(), key) != 0 &&  if_free_node == 0 )
                {
                    float nums_sum;
                    node.get_sum_ms(nums_sum);
                    if (nums_sum > max_nms)
                    ...
                    new_prob.push_back(probs[semantic]);
                    float inf_ent_new = 0.0;
                    for (int sem_no = 0; sem_no < SemanticOcTreeNode::num_class; sem_no ++ )
                    {
                        if (probs[sem_no] < 0.01 || probs[sem_no] > 0.99)
                            continue;
                        else
                            inf_ent_new = inf_ent_new - probs[sem_no] *  log(probs[sem_no]) / log(2);
                    }
                    // 淘汰2：信息熵
                    // new_ent.push_back(inf_ent_new);
                    // 淘汰1：自己定义的
                    // new_ent.push_back(1-probs[semantic] * log(nums_sum) /log(max_nms) );
                    // 正确：方差
                    std::vector<float> vars(SemanticOcTreeNode::num_class);
                    node.get_vars(vars);
                    new_ent.push_back(vars[semantic]);
                }
            }
        }

        // std::cout << "the size of common block is "<< common_block.size() << std::endl;
        // std::cout << "the size of common node is "<< common_node.size() << std::endl;

        // 接着只是对共有区域进行迭代过程, 方法为使用周围的block更新当前block所有体素，也就是全局到全局的扩散过程
        // for (int time_num = times-1; time_num >0 ; time_num--)
        // {
        //     std::cout << "Start time :  " << time_num << std::endl;
        //     // 先把此时全局体素的所有标签拿出来，也就是得到结点，防止迭代过程中改变，用大体素block代替
        //     for (auto block_key = common_block.begin(); block_key != common_block.end(); block_key++)
        //     {
        //         Block *block = block_arr[*block_key];
        //         std::vector<float> scs(SemanticOcTreeNode::num_class, 0.0);  //得到这个叶节点中每个体素的标签状态
        //         for (auto leaf_it = block->begin_leaf(); leaf_it != block->end_leaf(); ++leaf_it) //这个扩展block中的每个叶节点
        //         {                              
        //             std::vector<float> scs_node(SemanticOcTreeNode::num_class);  //得到这个叶节点中每个体素的标签状态
        //             leaf_it.get_node().get_ms(scs_node);
        //             for (int i = 0; i < SemanticOcTreeNode::num_class; i++)
        //             {
        //                 scs[i] = scs[i] + scs_node[i];
        //             }
        //         }
        //         scs_arr.push_back(scs);
        //         // std::cout << scs[1] << std::endl;
        //     }
        ...
        //     {
        //         Block *block = block_arr[*block_key];
        //         std::vector<float> scs = scs_arr[scs_num];
        //         point3f p_block = hash_key_to_block(*block_key); // block中心坐标
        //         ExtendedBlock eblock = block->get_extended_block();
        //         for (auto block_it = eblock.cbegin(); block_it != eblock.cend(); ++block_it) 
        //         {
        //             // 当前块不需要，因为本来就有，更新只需考虑周围体素
        //             if (*block_it == *block_key)
        //                 continue;
        //             if (block_arr.find(*block_it) == block_arr.end())  //如果周围没有block
        //             {
        //                 block_arr.emplace(*block_it, new Block(hash_key_to_block(*block_it)));  //就新建一个
        //                 new_block.push_back(*block_it); 
        //             }
        //             Block *block_ex =  block_arr[*block_it];
        //             for (auto leaf_it = block_ex->begin_leaf(); leaf_it != block_ex->end_leaf(); ++leaf_it) //这个扩展block中的每个叶节点
        //             {   
        //                 point3f p_leaf = block_ex->get_loc(leaf_it);     //整个block中这个叶节点的坐标          
        //                 float dis =  p_block.distance(p_leaf);   // 得到距离
        //                 dis = dis / SemanticOcTreeNode::ell ;
        //                 float k =  SemanticOcTreeNode::sf2 * (  ( 2 + cos (2.0f * 3.1415926f * dis)) * (1 - dis) /3  + sin( 2.0f * 3.1415926f *dis  ) / (2.0f * 3.1415926f) );
        //                 // std::cout << k  << std::endl;
        ...
        for (int time_num = times-1; time_num >0 ; time_num--)
        {
            ...
            {
                Block *block = block_arr[*block_key];
                for (auto leaf_it_c = block->begin_leaf(); leaf_it_c != block->end_leaf(); ++leaf_it_c) //这个扩展block中的每个叶节点
                {
                    std::vector<float> scs = scs_arr[scs_num];
                    point3f p_block = block->get_loc(leaf_it_c);     //整个block中这个叶节点的坐标    
                    ExtendedBlock eblock = block->get_extended_block();
                    for (auto block_it = eblock.cbegin(); block_it != eblock.cend(); ++block_it) 
                    {
                        // 当前块不需要，因为本来就有，更新只需考虑周围体素
        ...
        if (if_cout)
        {
            ...
            {  
                hd_ccsom::point3f p = common_node[i];
                ...
                float inf_ent_new_2 = 0.0;
                for (int sem_no = 0; sem_no < SemanticOcTreeNode::num_class; sem_no ++ )
                {
                    if (probs[sem_no] < 0.01 || probs[sem_no] > 0.99)
                        continue;
                    else
                        inf_ent_new_2 = inf_ent_new_2 - probs[sem_no] *  log(probs[sem_no]) / log(2);
                }
                // 淘汰2：信息熵
                // new_ent_after.push_back(inf_ent_new_2);
                // 淘汰1：自己定义的
                // new_ent_after.push_back(1-probs[semantic] * log(nums_sum) /log(1000) );
                //正确：方差
                std::vector<float> vars(SemanticOcTreeNode::num_class);
                node.get_vars(vars);
                new_ent_after.push_back(vars[semantic]);
            }
            
            // ******为了比较*********， 把所有结果整理出来
            for (int i = 0; i < size; i++)
            {
                OutFile<<std::setw(2)<<old_semantic[i]<<"    "<<std::fixed <<std::setprecision(4)<<old_ent[i]<<"    "<<old_prob[i]
                                <<"    "<<std::setw(2)<<old_semantic_2[i]<<"    "<<std::fixed <<std::setprecision(4)<<old_ent_2[i]<<"    "<<old_prob_2[i]
                                <<"    "<<std::setw(2)<<new_semantic[i]<<"    "<<std::fixed <<std::setprecision(4)<<new_ent[i]<<"    "<<new_prob[i]
                                <<"    "<<std::setw(2)<<new_semantic_after[i]<<"    "<<std::fixed <<std::setprecision(4)<<new_ent_after[i]<<"    "<<new_prob_after[i]<<"\n";
            }
            OutFile.close();
            std::cout << "the size of robot1 is "<< old_ent.size() << std::endl;
            std::cout << "the size of robot2 is "<< old_ent_2.size() << std::endl;
            std::cout << "the size of single is "<< new_ent.size() << std::endl;
            std::cout << "the size of multi is "<< new_ent_after.size() << std::endl;
            double sum_old = accumulate(old_ent.begin(), old_ent.end(), 0.0) ;
            double sum_old_2 = accumulate(old_ent_2.begin(), old_ent_2.end(), 0.0) ;
            double sum_new = accumulate(new_ent.begin(), new_ent.end(), 0.0) ;
            double sum_new_after = accumulate(new_ent_after.begin(), new_ent_after.end(), 0.0) ;
            float mean_old = sum_old  / size;   // 新的平均概率
            float mean_old_2 = sum_old_2  / size;   // 新的平均概率
            float mean_new = sum_new / size;   // 新的平均概率
            float mean_new_after = sum_new_after / size;   // 新的平均概率
            std::cout << "map1: " <<std::fixed <<std::setprecision(6)<< mean_old << std::endl;
            std::cout << "map2: " <<std::fixed <<std::setprecision(6)<< mean_old_2 << std::endl;
            std::cout << "global map: " <<std::fixed <<std::setprecision(6)<< mean_new << std::endl;
            std::cout << "global map after LD: " <<std::fixed <<std::setprecision(6)<< mean_new_after << std::endl ;
        }
    }
    ...
    int SemanticBKIOctoMap::block_is_unknow(const BlockHashKey &key) {
        ...
        {
            if( it.get_node().get_state() != hd_ccsom::State::UNKNOWN )
                return 0;
        }
        return 1;
    }
    ...
    void SemanticBKIOctoMap::print_vaule(void){
        std::ofstream OutFile("/home/dyn/catkin_ws/src/HD-CCSOM/stanford_result/conferenceRoom/dyn.txt"); 
        std::ofstream OutFile_pro("/home/dyn/catkin_ws/src/HD-CCSOM/stanford_result/conferenceRoom/dyn_pro.txt"); 
        int i = 0;
        // 为了准确使用int，然后除以100
        int dis = 25;    //dyn和bki的时候使用
        // int dis = 0;   //csm的时候使用
        // 会议室
        for(int x_int = 14000-dis; x_int < 20000-dis; x_int = x_int + resolution*1000)
        {
            for(int y_int = 500-dis; y_int < 6500-dis; y_int = y_int + resolution*1000)
            {
                for(int z_int = -200-dis; z_int < 3000-dis; z_int = z_int + resolution*1000)
                {
        // // 休息室
        ...
        //         {
        // // 办公室
        // for(int x_int = 16000-dis; x_int < 19800-dis; x_int = x_int + resolution*1000)
        // {
        //     for(int y_int = -5100-dis; y_int < -800-dis; y_int = y_int + resolution*1000)
        //     {
        //         for(int z_int = -200-dis; z_int < 3100-dis; z_int = z_int + resolution*1000)
        //         {
                    i++;
                    float x, y, z;
                    x = x_int/1000.0;
...
#pragma omp critical
                    ...
                    std::vector<float> probs(SemanticOcTreeNode::num_class);
                    node.get_probs(probs);
                    OutFile_pro  <<std::fixed <<std::setprecision(4) << 1-probs[0] << "\n"; 
                    if (node.get_state() == hd_ccsom::State::OCCUPIED)
                    {
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
    ...
    void SemanticBKIOctoMap::insert_pointcloud_see(const PCLPointCloud &cloud, const point3f &origin, float ds_resolution,
                                      float free_res, float max_range) {
...
#pragma omp parallel for
            ...
            else
            {
                ...
                //     {
                //         get_gp_points_in_bbox(*it, points); 
                //         // std::cout << points.size() << std::endl;
                //         for (auto it_poi = points.cbegin(); it_poi != points.cend(); ++it_poi) 
                //         {
                //             if (sem_num ==  -1)
                //             {
                //                 sem_num = it_poi->second;
                //                 // std::cout   <<  it_poi->second << std::endl;
                //             }
                //             else if (it_poi->second != sem_num)
                //             {
                //                 ok = 0;
                //                 std::cout << it_poi->second  << " "<< sem_num << std::endl;
                //             }
                //         }
                //     }
                // }
...
#pragma omp critical
#endif
            if (dis_ent>ent_thre && ok == 1){   //如果当前block的hit分布熵超过熵阈值
                // std::cout<<"dis_ent:    " << dis_ent<<std::endl;
